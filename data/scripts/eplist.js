...
function makeList(sortfn, series = true) {
    ...
    sortfn();
    eplist.innerHTML = "<ol>" + data.map(ep => `<li style='color:oklch(70% 100% ${Colour(ep)});'>${Names(ep, series)}<span class="hidden-info">,&nbsp;aired:&nbsp;${airdate(...parseDate(ep))} space: ${ep.location.space}</span></li>`).join("\n") + "</ol>";
}
