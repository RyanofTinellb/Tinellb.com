class Game {
  ...
  ensureSwap(cell) {
    ...
    if (cell.contents().length == 1) {
      let col = parseInt(cell.contents() - 1);
      let sqr = this.rows[row].cells[col];
      if (sqr.contents().length != 1) {
        this.set(sqr, cell.col + 1)
      }
    } else {
      console.log(cell.contents())
      cell.possibilities.forEach((elt, col) => {
        if (!elt) {
          let sqr = this.rows[row].cells[col]
          let len = sqr.contents().length;
          sqr.possibilities[cell.col] = false;
          sqr.update();
          if (len == 2 && sqr.contents().length == 1) {
            this.cleanCousins(sqr);
          }
        }
      });
    }
